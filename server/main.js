...
var sharedConsole = new Meteor.Collection('_clientConsole', {connection: null});
...
Meteor.publish("_clientConsole", function() {
  return sharedConsole.find();
})
...
Meteor.methods({
  runCode: function (name) {
    ...
    ls.stdout.on('data', Meteor.bindEnvironment((data)=>{
      //console.log(`${data}`);
      sharedConsole.insert({args:`${data}`});
    }));
